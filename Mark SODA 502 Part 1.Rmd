...
Now that we have a graph set up, we can swap out variables to plot different aspects. Here instead of cameo.root, we use countryname, which contains the country names. Of course, if we wanted to publish this we'd update the codes to full country names (more on that later!).

```{r NYT_histogram_country}
# Set as variable so we can modify titles + labels on the fly
binwidth <- 365
